...
const els = {
  moneyVal: $('moneyVal'), moneyMax: $('moneyMax'), moneyRate: $('moneyRate'),
    moneyBar: $('moneyBar'), walletBtn: $('walletBtn'),
      cannonBtn: $('cannonBtn'), cannonBar: $('cannonBar'),
        stageName: $('stageName'), speedBtn: $('speedBtn'), unitRow: $('unitRow'),
        };
        const unitBtns = [];
                                                                                                                ...
                                                                                                                unitBtns.forEach((b, i) => {
                                                                                                                    const cat = CATS[i];
                                                                                                                        const remain = Math.max(0, (game.cd[cat.id] || 0) - game.t);
                                                                                                                            b.querySelector('.cd').style.height = remain > 0 ? (remain / cat.cd * 100) + '%' : '0%';
                                                                                                                                b.classList.toggle('disabled', !inBattle || game.money < cat.cost || remain > 0 || !!game.resultPending);
                                                                                                                                  });
                                                                                                                                    const ratio = game.cannonCharge / CANNON.chargeTime;
                                                                                                                                      els.cannonBar.style.width = (ratio * 100) + '%';
                                                                                                                                        ...
                                                                                                                                        export function setStageName(text) { els.stageName.textContent = text; }
                                                                                                                                        export function updateSpeedBtn() { els.speedBtn.textContent = 'x' + game.speed; }